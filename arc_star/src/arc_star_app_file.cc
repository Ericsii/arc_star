...
int LoadEventFile(const std::string file_path, std::vector<Event> &event_vec,
  ...
  while (
    ...
    file >> t >> x >> y >> p; // Input format from the Event Camera Dataset.
                              // Adapt to your file format
...
int main(int argc, char *argv[]) {
  ...
  if (!(argc == 2 || argc == 3)) {
    std::cerr << "Usage: " << argv[0]
              << "  my_dataset_folder/events.txt "
                 "[my_result_folder/classification.txt]"
              << std::endl;
    return 1;
  }
  ...
  LoadEventFile(
      event_file_path, event_vec,
      0); // Load all the events (n_event=0). Note: if you expect to load a
          // massive amount of events consider pre-reserving larger amount
  ...
  // Output classification vector (true (1) - event-corner and false (0)
  // otherwise)
  ...
  const double elapsed_time =
      std::chrono::duration_cast<std::chrono::nanoseconds>(t_final - t_init)
          .count();
  const double percentage_corners =
      (corner_count / double(event_vec.size())) * 100;
  const double time_per_event =
      elapsed_time /
      event_vec.size(); // Average time to process one event [ns/ev]
  const double event_rate =
      1 / (time_per_event * 1e-3); // Average Event Rate [Million ev / s]
  const double real_time_factor =
      event_vec[event_vec.size() - 1].t / (elapsed_time * 1e-9);

  std::cout << "Corner classification stats: " << std::endl
            << "\t Num. Events: " << event_vec.size() << std::endl
            << "\t Num. Corners: " << corner_count << std::endl
            << "\t Reduction Percentage: " << percentage_corners << "%"
            << std::endl
            << std::endl
            << "Timing stats: " << std::endl
            << " \tAvg. Time per event: " << time_per_event << " [ns/ev]"
            << std::endl
            << " \tAvg. Max. Event Rate: " << event_rate << " [Mev/s]"
            << std::endl
            << " \tAvg. Real-time factor: " << real_time_factor << "x"
            << std::endl
            << std::endl;
